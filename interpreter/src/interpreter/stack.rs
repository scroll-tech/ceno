use crate::{
    primitives::{B256, U256},
    InstructionResult,
};
use alloc::vec::Vec;
use core::fmt;

/// EVM interpreter stack limit.
pub const STACK_LIMIT: usize = 1024;

/// EVM stack with [STACK_LIMIT] capacity of words.
#[derive(Debug, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serde", derive(serde::Serialize))]
pub struct Stack {
    /// The underlying data of the stack.
    data: Vec<U256>,
    timestamps: Vec<u64>,
}

impl fmt::Display for Stack {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("[")?;
        for ((i, x), ts) in self.data.iter().enumerate().zip(self.timestamps.iter()) {
            if i > 0 {
                f.write_str(", ")?;
            }
            write!(f, "({x}: {ts})")?;
        }
        f.write_str("]")
    }
}

impl Default for Stack {
    #[inline]
    fn default() -> Self {
        Self::new()
    }
}

impl Stack {
    /// Instantiate a new stack with the [default stack limit][STACK_LIMIT].
    #[inline]
    pub fn new() -> Self {
        Self {
            // SAFETY: expansion functions assume that capacity is `STACK_LIMIT`.
            data: Vec::with_capacity(STACK_LIMIT),
            timestamps: Vec::with_capacity(STACK_LIMIT),
        }
    }

    /// Returns the length of the stack in words.
    #[inline]
    pub fn len(&self) -> usize {
        self.data.len()
    }

    /// Returns whether the stack is empty.
    #[inline]
    pub fn is_empty(&self) -> bool {
        self.data.is_empty()
    }

    /// Returns the underlying data of the stack.
    #[inline]
    pub fn data(&self) -> &Vec<U256> {
        &self.data
    }

    /// Returns the underlying timestamp of the stack.
    #[inline]
    pub fn timestamps(&self) -> &Vec<u64> {
        &self.timestamps
    }

    /// Consumes the stack and returns the underlying data.
    #[inline]
    pub fn into_data(self) -> Vec<U256> {
        self.data
    }

    /// Consumes the stack and returns the underlying timestamp.
    #[inline]
    pub fn into_timestamps(self) -> Vec<u64> {
        self.timestamps
    }

    /// Removes the topmost element from the stack and returns it, or `StackUnderflow` if it is
    /// empty.
    #[inline]
    pub fn pop(&mut self) -> Result<(U256, u64), InstructionResult> {
        let x = self.data.pop().ok_or(InstructionResult::StackUnderflow)?;
        let ts = self.timestamps.pop().unwrap();
        Ok((x, ts))
    }

    /// Removes the topmost element from the stack and returns it.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop_unsafe(&mut self) -> (U256, u64) {
        let x = self.data.pop().unwrap();
        let ts = self.timestamps.pop().unwrap();
        (x, ts)
    }

    /// Peeks the top of the stack.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn top_unsafe(&mut self) -> (&mut U256, &mut u64) {
        let len = self.data.len();
        let x = self.data.get_unchecked_mut(len - 1);
        let ts = self.timestamps.get_unchecked_mut(len - 1);
        (x, ts)
    }

    /// Pop the topmost value, returning the value and the new topmost value.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop_top_unsafe(&mut self) -> ((U256, u64), (&mut U256, &mut u64)) {
        let pop = self.pop_unsafe();
        let top = self.top_unsafe();
        (pop, top)
    }

    /// Pops 2 values from the stack.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop2_unsafe(&mut self) -> ((U256, u64), (U256, u64)) {
        let pop1 = self.pop_unsafe();
        let pop2 = self.pop_unsafe();
        (pop1, pop2)
    }

    /// Pops 2 values from the stack and returns them, in addition to the new topmost value.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop2_top_unsafe(&mut self) -> ((U256, u64), (U256, u64), (&mut U256, &mut u64)) {
        let pop1 = self.pop_unsafe();
        let pop2 = self.pop_unsafe();
        let top = self.top_unsafe();

        (pop1, pop2, top)
    }

    /// Pops 3 values from the stack.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop3_unsafe(&mut self) -> ((U256, u64), (U256, u64), (U256, u64)) {
        let pop1 = self.pop_unsafe();
        let pop2 = self.pop_unsafe();
        let pop3 = self.pop_unsafe();

        (pop1, pop2, pop3)
    }

    /// Pops 4 values from the stack.
    ///
    /// # Safety
    ///
    /// The caller is responsible for checking the length of the stack.
    #[inline]
    pub unsafe fn pop4_unsafe(&mut self) -> ((U256, u64), (U256, u64), (U256, u64), (U256, u64)) {
        let pop1 = self.pop_unsafe();
        let pop2 = self.pop_unsafe();
        let pop3 = self.pop_unsafe();
        let pop4 = self.pop_unsafe();

        (pop1, pop2, pop3, pop4)
    }

    /// Push a new value into the stack. If it will exceed the stack limit,
    /// returns `StackOverflow` error and leaves the stack unchanged.
    #[inline]
    pub fn push_b256(&mut self, value: B256, ts: u64) -> Result<(), InstructionResult> {
        self.push(value.into(), ts)
    }

    /// Push a new value onto the stack.
    ///
    /// If it will exceed the stack limit, returns `StackOverflow` error and leaves the stack
    /// unchanged.
    #[inline]
    pub fn push(&mut self, value: U256, ts: u64) -> Result<(), InstructionResult> {
        // allows the compiler to optimize out the `Vec::push` capacity check
        assume!(self.data.capacity() == STACK_LIMIT);
        if self.data.len() == STACK_LIMIT {
            return Err(InstructionResult::StackOverflow);
        }
        self.data.push(value);
        self.timestamps.push(ts);
        Ok(())
    }

    /// Peek a value at given index for the stack, where the top of
    /// the stack is at index `0`. If the index is too large,
    /// `StackError::Underflow` is returned.
    #[inline]
    pub fn peek(&self, no_from_top: usize) -> Result<(U256, u64), InstructionResult> {
        if self.data.len() > no_from_top {
            Ok((
                self.data[self.data.len() - no_from_top - 1],
                self.timestamps[self.timestamps.len() - no_from_top - 1],
            ))
        } else {
            Err(InstructionResult::StackUnderflow)
        }
    }

    /// Duplicates the `N`th value from the top of the stack.
    #[inline]
    pub fn dup<const N: usize>(&mut self, ts: u64) -> Result<(U256, u64), InstructionResult> {
        let len = self.data.len();
        if len < N {
            Err(InstructionResult::StackUnderflow)
        } else if len + 1 > STACK_LIMIT {
            Err(InstructionResult::StackOverflow)
        } else {
            // SAFETY: check for out of bounds is done above and it makes this safe to do.
            let (old_value, old_ts) = unsafe {
                let data = self.data.as_mut_ptr();
                core::ptr::copy_nonoverlapping(data.add(len - N), data.add(len), 1);
                self.data.set_len(len + 1);
                let timestamp = self.timestamps.as_mut_ptr();
                let old_ts = core::ptr::replace(timestamp.add(len - N), ts);
                core::ptr::write(timestamp.add(len), ts);
                self.timestamps.set_len(len + 1);
                (self.data[len - N], old_ts)
            };
            Ok((old_value, old_ts))
        }
    }

    /// Swaps the topmost value with the `N`th value from the top.
    #[inline]
    pub fn swap<const N: usize>(
        &mut self,
        ts: u64,
    ) -> Result<(U256, u64, U256, u64), InstructionResult> {
        let len = self.data.len();
        if len <= N {
            return Err(InstructionResult::StackUnderflow);
        }
        let last = len - 1;
        let (old_data_0, old_data_1) = (self.data[last], self.data[last - N]);
        let (old_ts_0, old_ts_1) = (self.timestamps[last], self.timestamps[last - N]);
        self.data.swap(last, last - N);
        self.timestamps[last - N] = ts;
        self.timestamps[last] = ts;
        Ok((old_data_0, old_ts_0, old_data_1, old_ts_1))
    }

    /// Pushes an arbitrary length slice of bytes onto the stack, padding the last word with zeros
    /// if necessary.
    #[inline]
    pub fn push_slice(&mut self, slice: &[u8], ts: u64) -> Result<(), InstructionResult> {
        if slice.is_empty() {
            return Ok(());
        }

        let n_words = (slice.len() + 31) / 32;
        let new_len = self.data.len() + n_words;
        if new_len > STACK_LIMIT {
            return Err(InstructionResult::StackOverflow);
        }

        // SAFETY: length checked above.
        unsafe {
            let dst_data = self.data.as_mut_ptr().add(self.data.len()).cast::<u64>();
            let dst_ts = self.timestamps.as_mut_ptr().add(self.timestamps.len());
            self.data.set_len(new_len);
            self.timestamps.set_len(new_len);

            // write timestamps
            for i in 0..n_words {
                core::ptr::write(dst_ts.add(i), ts);
            }

            let mut i = 0;

            // write full words
            let words = slice.chunks_exact(32);
            let partial_last_word = words.remainder();
            for word in words {
                // Note: we unroll `U256::from_be_bytes` here to write directly into the buffer,
                // instead of creating a 32 byte array on the stack and then copying it over.
                for l in word.rchunks_exact(8) {
                    dst_data
                        .add(i)
                        .write(u64::from_be_bytes(l.try_into().unwrap()));
                    i += 1;
                }
            }

            if partial_last_word.is_empty() {
                return Ok(());
            }

            // write limbs of partial last word
            let limbs = partial_last_word.rchunks_exact(8);
            let partial_last_limb = limbs.remainder();
            for l in limbs {
                dst_data
                    .add(i)
                    .write(u64::from_be_bytes(l.try_into().unwrap()));
                i += 1;
            }

            // write partial last limb by padding with zeros
            if !partial_last_limb.is_empty() {
                let mut tmp = [0u8; 8];
                tmp[8 - partial_last_limb.len()..].copy_from_slice(partial_last_limb);
                dst_data.add(i).write(u64::from_be_bytes(tmp));
                i += 1;
            }

            debug_assert_eq!((i + 3) / 4, n_words, "wrote too much");

            // zero out upper bytes of last word
            let m = i % 4; // 32 / 8
            if m != 0 {
                dst_data.add(i).write_bytes(0, 4 - m);
            }
        }

        Ok(())
    }

    /// Set a value at given index for the stack, where the top of the
    /// stack is at index `0`. If the index is too large,
    /// `StackError::Underflow` is returned.
    #[inline]
    pub fn set(&mut self, no_from_top: usize, val: U256, ts: u64) -> Result<(), InstructionResult> {
        if self.data.len() > no_from_top {
            let len = self.data.len();
            self.data[len - no_from_top - 1] = val;
            self.timestamps[len - no_from_top - 1] = ts;
            Ok(())
        } else {
            Err(InstructionResult::StackUnderflow)
        }
    }
}

#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for Stack {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let mut data = Vec::<U256>::deserialize(deserializer)?;
        if data.len() > STACK_LIMIT {
            return Err(serde::de::Error::custom(alloc::format!(
                "stack size exceeds limit: {} > {}",
                data.len(),
                STACK_LIMIT
            )));
        }
        data.reserve(STACK_LIMIT - data.len());
        Ok(Self { data })
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn run(f: impl FnOnce(&mut Stack)) {
        let mut stack = Stack::new();
        // fill capacity with non-zero values
        unsafe {
            stack.data.set_len(STACK_LIMIT);
            stack.data.fill(U256::MAX);
            stack.data.set_len(0);
        }
        f(&mut stack);
    }

    #[test]
    fn push_slices() {
        // no-op
        let ts: u64 = 0;
        run(|stack| {
            stack.push_slice(b"", ts).unwrap();
            assert_eq!(stack.data, []);
        });

        // one word
        run(|stack| {
            stack.push_slice(&[42], ts).unwrap();
            assert_eq!(stack.data, [U256::from(42)]);
            assert_eq!(stack.timestamps, [0]);
            assert_eq!(ts, 1);
        });

        let n = 0x1111_2222_3333_4444_5555_6666_7777_8888_u128;
        run(|stack| {
            stack.push_slice(&n.to_be_bytes(), ts).unwrap();
            assert_eq!(stack.data, [U256::from(n)]);
            assert_eq!(stack.timestamps, [0]);
            assert_eq!(ts, 1);
        });

        // more than one word
        run(|stack| {
            let b = [U256::from(n).to_be_bytes::<32>(); 2].concat();
            stack.push_slice(&b, ts).unwrap();
            assert_eq!(stack.data, [U256::from(n); 2]);
            assert_eq!(stack.timestamps, [0, 1]);
            assert_eq!(ts, 2);
        });

        run(|stack| {
            let b = [&[0; 32][..], &[42u8]].concat();
            stack.push_slice(&b, ts).unwrap();
            assert_eq!(stack.data, [U256::ZERO, U256::from(42)]);
            assert_eq!(stack.timestamps, [0, 1]);
            assert_eq!(ts, 2);
        });

        run(|stack| {
            let b = [&[0; 32][..], &n.to_be_bytes()].concat();
            stack.push_slice(&b, ts).unwrap();
            assert_eq!(stack.data, [U256::ZERO, U256::from(n)]);
            assert_eq!(stack.timestamps, [0, 1]);
            assert_eq!(ts, 2);
        });

        run(|stack| {
            let b = [&[0; 64][..], &n.to_be_bytes()].concat();
            stack.push_slice(&b, ts).unwrap();
            assert_eq!(stack.data, [U256::ZERO, U256::ZERO, U256::from(n)]);
            assert_eq!(stack.timestamps, [0, 1, 2]);
            assert_eq!(ts, 3);
        });
    }
}
