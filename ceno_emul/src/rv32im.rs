...
impl EncodedInstruction {
    pub fn encode(kind: InsnKind, rs1: u32, rs2: u32, rd: u32, imm: u32) -> u32 {
        match kind.codes().format {
            InsnFormat::R => Self::encode_r(kind, rs1, rs2, rd),
            InsnFormat::I => Self::encode_i(kind, rs1, rd, imm),
            InsnFormat::S => Self::encode_s(kind, rs1, rs2, imm),
            InsnFormat::B => Self::encode_b(kind, rs1, rs2, imm),
            InsnFormat::U => Self::encode_u(kind, rd, imm),
            InsnFormat::J => Self::encode_j(kind, rd, imm),
        }
    }

    fn encode_r(kind: InsnKind, rs1: u32, rs2: u32, rd: u32) -> u32 {
        let rs2 = rs2 & 0x1f; // 5-bits mask
        let rs1 = rs1 & 0x1f;
        let rd = rd & 0x1f;
    ...
    fn encode_i(kind: InsnKind, rs1: u32, rd: u32, imm: u32) -> u32 {
        ...
        let func3 = kind.codes().func3;
        let opcode = kind.codes().opcode;
        let imm = imm & 0x0fff; // 12-bits mask
    ...
    fn encode_s(kind: InsnKind, rs1: u32, rs2: u32, imm: u32) -> u32 {
        ...
        let opcode = kind.codes().opcode;
        let imm_lo = imm & 0x1f;
        let imm_hi = (imm >> 5) & 0x7f; // 7-bits mask
    ...
    fn encode_u(kind: InsnKind, rd: u32, imm: u32) -> u32 {
        (imm >> 12) << 12 | (rd & 0x1f) << 7 | kind.codes().opcode
    }
}
