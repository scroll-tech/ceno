...
pub struct Secp256r1DecompressSpec;
...
impl SyscallSpec for Secp256r1AddSpec {
    ...
    const CODE: u32 = ceno_rt::syscalls::SECP256R1_ADD;
}
...
impl SyscallSpec for Secp256r1DoubleSpec {
    ...
    const CODE: u32 = ceno_rt::syscalls::SECP256R1_DOUBLE;
}

impl SyscallSpec for Secp256r1DecompressSpec {
    const NAME: &'static str = "SECP256R1_DECOMPRESS";

    const REG_OPS_COUNT: usize = 2;
    const MEM_OPS_COUNT: usize = 2 * COORDINATE_WORDS;
    const CODE: u32 = ceno_rt::syscalls::SECP256R1_DECOMPRESS;
}
...
/// Wrapper type for a maybe-point on the secp256r1 curve that implements conversions
/// from and to VM word-representations according to the syscall spec
pub struct SecpMaybePoint(pub secp::MaybePoint);

impl From<SecpMaybePoint> for [Word; SECP256R1_ARG_WORDS] {
    fn from(maybe_point: SecpMaybePoint) -> [Word; SECP256R1_ARG_WORDS] {
...
pub fn secp256r1_add(vm: &VMState) -> SyscallEffects {
    ...
    let [mut p_view, q_view] =
        [p_ptr, q_ptr].map(|start| MemoryView::<SECP256R1_ARG_WORDS>::new(vm, start));
    ...
    let output_words: [Word; SECP256R1_ARG_WORDS] = sum.into();
...
pub fn secp256r1_double(vm: &VMState) -> SyscallEffects {
    let p_ptr = vm.peek_register(Platform::reg_arg0());
    ...
    // P's memory segment
    let mut p_view = MemoryView::<SECP256R1_ARG_WORDS>::new(vm, p_ptr);
    ...
    let output_words: [Word; SECP256R1_ARG_WORDS] = result.into();
...
/// Trace the execution of a secp256r1_decompress call
pub fn secp256r1_decompress(vm: &VMState) -> SyscallEffects {
