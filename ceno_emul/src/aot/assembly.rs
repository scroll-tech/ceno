...
pub(super) fn emit_skeleton_l1_shared_recorder(mut file: impl Write) -> Result<()> {
    writeln!(
        file,
        r#"
...
pub(super) fn emit_compact_skeleton_l1c_shared_recorder(mut file: impl Write) -> Result<()> {
    writeln!(
        file,
        r#"
...
pub(super) fn emit_compact_values_l2c_shared_recorder(mut file: impl Write) -> Result<()> {
    writeln!(
        file,
        r#"
...
pub(super) fn emit_l3c_commit(mut file: impl Write, size: usize) -> Result<()> {
    writeln!(
        file,
        "    movq {AOT_CTX_COMPACT_BYTES_CURSOR_OFFSET}(%r12), %r8"
    )?;
...
pub(super) fn emit_l4c_update_bounds(mut file: impl Write, label: &str) -> Result<()> {
...
pub(super) fn emit_l5c_consume_row_future_accesses(mut file: impl Write) -> Result<()> {
    writeln!(
        file,
        r#"
    xorl %esi, %esi
...
pub(super) fn emit_compact_registers_l3c_shared_recorder(
    mut file: impl Write,
    memory_l4: bool,
    future_l5: bool,
) -> Result<()> {
...
pub(super) fn emit_l7_block_family_admission(
    mut file: impl Write,
    program: &Program,
    block: &BasicBlock,
) -> Result<()> {
...
pub(super) fn emit_gpu_replay_packed_block_admission(
    mut file: impl Write,
    program: &Program,
    block: &BasicBlock,
) -> Result<()> {
...
pub(super) fn l7_block_family_rank(
    program: &Program,
    block_start_pc: u32,
    block_end_pc: u32,
    current_pc: u32,
...
#[derive(Clone)]
pub(super) struct GpuPackedStaticField {
    load_eax: String,
    width: usize,
}

pub(super) fn emit_gpu_replay_static_pack_stores(
    mut file: impl Write,
    ...
    while chunk_start < total_bits {
        ...
        match chunk_bits.div_ceil(8) {
            ...
            1 => writeln!(file, "    movb %r8b, {byte_offset}(%r9)")?,
            2 => writeln!(file, "    movw %r8w, {byte_offset}(%r9)")?,
            3 => {
                writeln!(file, "    movw %r8w, {byte_offset}(%r9)")?;
                writeln!(file, "    shrq $16, %r8")?;
                writeln!(file, "    movb %r8b, {}(%r9)", byte_offset + 2)?;
            }
            5 => {
                writeln!(file, "    movl %r8d, {byte_offset}(%r9)")?;
                writeln!(file, "    shrq $32, %r8")?;
                writeln!(file, "    movb %r8b, {}(%r9)", byte_offset + 4)?;
            }
            6 => {
                writeln!(file, "    movl %r8d, {byte_offset}(%r9)")?;
                writeln!(file, "    shrq $32, %r8")?;
            ...
            tail => bail!("unsupported packed static tail of {tail} bytes"),
        }
...
pub(super) fn emit_gpu_replay_packed_static_row(
    mut file: impl Write,
    program: &Program,
    pc: u32,
    insn: Instruction,
    ...
    let access = |previous_offset: usize, value_offset: usize| {
        [
            source(
                format!("movl {previous_offset}(%r12), %eax"),
                crate::gpu_typed_ingress::GPU_COMPACT_CYCLE_BITS,
            ),
...
pub(super) fn layered_register_predecessors(
    program: &Program,
    block_start_pc: u32,
    current_pc: u32,
...
pub(super) fn emit_layered_register_predecessors(
    mut file: impl Write,
    program: &Program,
    pc: u32,
    reserved: ReservedBlockStep,
...
pub(super) fn emit_gpu_replay_register_predecessors(
    mut file: impl Write,
    program: &Program,
    pc: u32,
    reserved: ReservedBlockStep,
...
pub(super) fn emit_layered_register_block_commit(
    mut file: impl Write,
    program: &Program,
    block: &BasicBlock,
) -> Result<()> {
...
pub(super) fn emit_gpu_replay_register_block_commit(
    mut file: impl Write,
    program: &Program,
    block: &BasicBlock,
) -> Result<()> {
...
pub(super) fn emit_rollback_gpu_replay_packed_steps(
    mut file: impl Write,
    trace_style: AssemblyTraceStyle,
    reserved_step: Option<ReservedBlockStep>,
) -> Result<()> {
...
pub(super) fn emit_restore_gpu_replay_packed_steps(
    mut file: impl Write,
    trace_style: AssemblyTraceStyle,
    reserved_step: Option<ReservedBlockStep>,
) -> Result<()> {
...
pub(super) fn emit_gpu_replay_packed_family_rollback(
    mut file: impl Write,
    program: &Program,
    pc: u32,
    insn: Instruction,
...
pub(super) fn emit_gpu_replay_packed_family_restore(
    mut file: impl Write,
    pc: u32,
