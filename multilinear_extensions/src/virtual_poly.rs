...
impl<'a, E: ExtensionField> VirtualPolynomial<'a, E> {
    ...
    pub fn mul_by_mle(&mut self, mle: ArcMultilinearExtension<'a, E>, coefficient: E) {
    ...
    pub fn to_ext_field(&self) -> VirtualPolynomial<E> {
        ...
        for mle in self.flattened_ml_extensions.iter() {
            ...
            let mle_ext_field = mle.get_base_field_vec().to_vec().into_mle();
            let mle_ext_field: ArcMultilinearExtension<E> = Arc::new(mle_ext_field);
            let mle_ext_field_ptr = Arc::as_ptr(&mle_ext_field) as *const () as usize;
    ...
    pub fn random(
        ...
        for _ in 0..num_products {
            ...
            let (product, product_sum) =
                DenseMultilinearExtension::random_mle_list(nv, num_multiplicands, &mut rng);
