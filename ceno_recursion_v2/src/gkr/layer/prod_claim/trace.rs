...
pub struct GkrProdSumCheckClaimTraceGenerator;

impl RowMajorChip<F> for GkrProdSumCheckClaimTraceGenerator {
    // (gkr_layer_records, mus)
            ...
            .for_each(|(proof_idx, (chunk, (record, mus_values)))| {
                ...
                chunk
                    ...
                    .for_each(|(layer_idx, row)| {
                        let cols: &mut GkrProdSumCheckClaimCols<F> = row.borrow_mut();
                        let num_prod = record.prod_count_at(layer_idx);
                        cols.is_enabled = F::ONE;
                        ...
                        cols.num_prod_count = F::from_usize(num_prod);

                        let acc_sum_with_cur = acc_sum + p_xi * pow_lambda;
