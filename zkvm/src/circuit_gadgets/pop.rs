...
impl<F: SmallField> OpcodeCircuitBuilder<F> {
    pub fn pop_basic(challenge: usize) -> Self {
        ...
        circuit_builder.add(next_stack_top[0], stack_top[0], ConstantType::Field(F::ONE));
        ...
        circuit_builder.add(stack_pop[0], wit[0], ConstantType::Field(F::ONE));
        ...
        let layout_in: HashMap<OpcodeLayoutIn, WireInIndex> = vec![
            ...
            (OpcodeLayoutIn::StackPop, stack_pop_idx),
        ]
        .into_iter()
        .collect();
        let layout_out: HashMap<OpcodeLayoutOut, WireOutIndex> = vec![
    ...
    pub fn pop_pro(challenge: usize) -> Self {
        todo!()
    }
}
