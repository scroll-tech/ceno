use gkr_graph::structs::{CircuitGraph, CircuitGraphWitness};
...
struct SingerProCircuit<F: SmallField>(CircuitGraph<F>);

struct SingerProWitness<F: SmallField>(CircuitGraphWitness<F>);

struct SingerProPublicIO<F: SmallField> {
    bytecode: Vec<F>,
    public_input: Vec<F>,
}

impl<F: SmallField> ZKVM<F> for SingerPro {
    type Circuit = SingerProCircuit<F>;

    type CircuitWitness = SingerProWitness<F>;

    type PublicIO = SingerProPublicIO<F>;
