...
### 2026-08-09: Execution floors and first exact planner-region attempt

- **Observed measurement:** rebuilding the retained ABI 50 source on the current machine produced a same-session control of 8.757104488 seconds total, 7.095058207 seconds native, and 1.662046281 seconds fallback. It preserved the required hash, 994,896,527 instructions, 16,865,461 events, and all 35 shard boundaries. This is slower than the earlier 8.36702-second warm result and confirms material machine/governor noise; causal comparisons below use the same session.
- **Controlled causal result:** a diagnostic planner no-op reduced total time to 7.603222683 seconds and native time to 5.494769803 seconds. It was intentionally invalid (15 shards and 16,156,909 events), so it was reverted. The approximately 1.600-second native reduction is a causal upper bound for the current planner path, not a shippable optimization. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/trim-planner/cold.log`.
- **Controlled causal result:** bypassing direct access/event bookkeeping while retaining cycles and packed stamps produced 8.209538702 seconds total, 6.547497812 seconds native, and 1.662040890 seconds fallback. The trim retained 35 planner boundaries but was trace-invalid (17,321 events), and was reverted. The same-session native ceiling was approximately 0.548 seconds. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/trim-access/cold.log`.
- **Controlled causal result:** additionally bypassing packed-stamp writes produced 7.823598854 seconds total, 6.103055764 seconds native, and 1.720543090 seconds fallback. It retained 35 planner boundaries but was trace-invalid (108,087 events), and was reverted. Packed stamp traffic therefore accounted for approximately another 0.444 seconds on the access-bookkeeping floor. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/trim-access-stamp/cold.log`.
- **Failed hypothesis:** ABI 52 grouped up to 16 adjacent, deterministic, register-only basic blocks, reserved their instruction/event budgets once, and used one aggregate cost descriptor when a conservative cell/cycle envelope proved the region safe. Any first-shard region, empty shard, failed envelope, or aggregate split retried the original exact per-block path. The focused AOT suite passed (45 passed, 1 ignored), and the block remained exact: required hash, 994,896,527 instructions, 16,865,461 events, FullTracer replay, and the byte-identical 35-boundary vector.
- **Observed measurement:** the ABI 52 cold result was 8.747497271 seconds total, 7.082665829 seconds native, and 1.664831442 seconds fallback. Its cached repeat was 8.691788981 seconds total, 7.049299501 seconds native, and 1.642489480 seconds fallback. This did not reduce the expected native planner signal or clear the retention gate, so the entire implementation and ABI bump were reverted. Artifacts: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/planner-regions-abi52/`.
- **Unresolved inference:** the planner floor remains worth pursuing, but deterministic register-only adjacency covers too little of the executed hot graph. A wider design needs measured dynamic region coverage and must include hot conditional and memory blocks while preserving exact prefix order near the 35 actual splits.
- **Failed intermediate hypothesis:** ABI 53 added generation-aware per-chip bucket ceilings but used a read-only proof scan followed by a second count-commit scan. It was exact and showed that 136,657,901 of 144,149,420 planned block entries (94.8%) stayed within their current cost buckets; 7,491,519 used the original exact transition after a callback invalidation or bucket crossing. Despite proving the intended signal, the duplicate stable-path scan regressed execution to 9.163339535 seconds total, 7.416643156 seconds native, and 1.746696379 seconds fallback. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/planner-bucket-abi53/`.
- **Controlled causal result:** ABI 54 commits instance counts during the proof scan and rolls back only already-scanned contributions when the rare exact path is required. Rust fallback, direct-syscall, and helper entries advance a generation; a chip whose cached ceiling predates that generation cannot take the fast path. The cold run was 8.291894062 seconds total, 6.632030489 seconds native, and 1.659863573 seconds fallback. The cached repeat was 8.242909286 seconds total, 6.605144175 seconds native, and 1.637765111 seconds fallback. Against the same-session retained ABI 50 control, this reduces total time by 0.514 seconds and native time by 0.490 seconds on the cached comparison, clearing the retention gate.
- **Observed measurement:** ABI 54 retained the same 136,657,901 fast and 7,491,519 exact planned-block counts, required hash, 994,896,527 instructions, 16,865,461 events, FullTracer replay, and byte-identical 35-boundary vector. Focused AOT validation passed 45 tests with 1 ignored. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/planner-bucket-abi54/`.
- **Observed measurement:** a fresh ABI 54 cached `perf record` run was 8.255271730 seconds total, 6.618603202 seconds native, and 1.636668528 seconds fallback. Within generated-code samples, accounting fell from the prior 20.1% to 16.9%, while memory was 47.0%, guards 13.6%, register first/latest 6.0% combined, plan commit 1.2%, and dispatch 2.6%. This confirms that the retained bucket cache reduced its intended region; it did not merely displace all of the cost back into accounting. Profile: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/planner-bucket-abi54/perf.data`; report: `perf-report.txt` in the same directory.
- **Failed hypothesis:** ABI 55 kept planner-count, bucket, latest-access, cycle, shard-start, and guest-memory bases in XMM lanes and reloaded them after every Rust callback. It remained exact and its cold sample was 7.965551876 seconds, but five cached samples were 8.271730247, 8.255691526, 8.289062389, 8.207217960, and 8.238750533 seconds (median 8.255691526). Median native time was 6.619080369 seconds, indistinguishable from ABI 54's 6.618603202-second profiled sample. The cold apparent win was noise; the stage missed the retention gate and was reverted. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/resident-bases-abi55/`.
- **Observed measurement / instrumentation cleanup:** ABI 56 removed the per-block fast/slow diagnostic increments after ABI 54 established their coverage. It remained exact and measured 8.243645753 seconds cold and 8.262492056 seconds cached, so the 144,149,420 counter increments were not a material elapsed-time bottleneck. The counters remain recorded in the ABI 54 artifacts rather than production generated code. Artifact: `/home/wusm/rust/ceno-reth-benchmark/.codex-results/aot-below5-20260809/planner-bucket-abi56/`.
