...
fn test_multiple_opcode() {
    ...
    let _add_config = cs.namespace("add", |cs| {
        AddInstruction::construct_circuit(&mut CircuitBuilder::<E>::new(cs))
    });
    let _sub_config = cs.namespace("sub", |cs| {
        SubInstruction::construct_circuit(&mut CircuitBuilder::<E>::new(cs))
    });
