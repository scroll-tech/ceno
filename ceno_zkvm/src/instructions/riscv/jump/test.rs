...
use crate::{
    ...
    scheme::mock_prover::{
        MOCK_PC_AUIPC, MOCK_PC_JAL, MOCK_PC_JALR, MOCK_PC_LUI, MOCK_PROGRAM, MockProver,
    },
};
...
fn test_opcode_jalr() {
    ...
    let new_pc: ByteAddr = ByteAddr(rs1_read.wrapping_add_signed(imm) & 0xfffe);
    let (raw_witin, lkm) = JalrInstruction::<GoldilocksExt2>::assign_instances(
        &config,
        cb.cs.num_witin as usize,
        vec![StepRecord::new_i_instruction(
            4,
            Change::new(MOCK_PC_JALR, new_pc),
            MOCK_PROGRAM[27],
            rs1_read,
            Change::new(0, (MOCK_PC_JALR + PC_STEP_SIZE).into()),
            0,
        )],
    )
    .unwrap();
