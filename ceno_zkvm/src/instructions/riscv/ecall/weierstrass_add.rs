    ...
    for WeierstrassAddAssignInstruction<E, EC>
{
    ...
    fn assign_instances(
        ...
        // second pass
        let instances: Vec<EllipticCurveAddInstance<EC::BaseField>> = tracing::info_span!(
                    ...
                    p.and_then(|p| q.map(|q| EllipticCurveAddInstance::<EC::BaseField> { p, q }))
                        .map_err(|_| {
                            ZKVMError::InvalidWitness(
                                "Failed to parse EllipticCurveAddInstance".into(),
                            )
                        })
                })
                .collect::<Result<_, _>>()
        ...
        let gpu_records = if use_gpu_relations {
            ...
            Some(
                tracing::info_span!("secp256k1_gpu_pack", operation = "add", n = instances.len())
                    .in_scope(|| {
                        WeierstrassAddAssignLayout::<E, EC>::compact_secp256k1_gpu_records(
                            &instances, affine,
                        )
                    }),
            )
