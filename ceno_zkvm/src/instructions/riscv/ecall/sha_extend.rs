...
use ceno_emul::{
    Change, InsnKind, PC_STEP_SIZE, Platform, SHA_EXTEND, StepRecord, WORD_SIZE, WriteOp,
};
...
use crate::{
    ...
    precompiles::{
        SHA_EXTEND_ROTATION_LOG2, SHA_EXTEND_ROTATION_ROUNDS, SHA_EXTEND_ROUNDS, ShaExtendInstance,
        ShaExtendLayout, ShaExtendTrace, ShaExtendWitInstance,
    },
...
impl<E: ExtensionField> Instruction<E> for ShaExtendInstruction<E> {
    ...
    fn build_gkr_iop_circuit(
        ...
        let vm_state = StateInOut::construct_circuit(cb, true)?;

        let Some(sel_last) = layout.selector_type_layout.sel_last.as_ref() else {
            return Err(ZKVMError::CircuitError);
        };
        cb.condition_require_equal(
            || "sha extend next pc",
            sel_last.selector_expr().expr(),
            vm_state.next_pc.unwrap().expr(),
            vm_state.pc.expr() + PC_STEP_SIZE,
            vm_state.pc.expr(),
        )?;
        ...
        let round_i = &layout.layer_exprs.wits.round_i;
        let offset = [
            round_i.expr() - 2,
            round_i.expr() - 7,
            round_i.expr() - 15,
        ...
        let mut mem_rw = izip!(offset, &layout.input32_exprs)
            .map(|(offset, val_before)| {
                WriteMEM::construct_circuit(
                    cb,
                    state_ptr.prev_value.as_ref().unwrap().value() + offset * WORD_SIZE,
                    val_before.clone(),
                    val_before.clone(),
                    vm_state.ts,
                )
        ...
        mem_rw.push(WriteMEM::construct_circuit(
            cb,
            state_ptr.prev_value.as_ref().unwrap().value() + round_i.expr() * WORD_SIZE,
    ...
    fn assign_instances(
        ...
        let num_instances = steps.len() / SHA_EXTEND_ROUNDS;
        ...
        raw_witin_iter
            .zip_eq(steps.par_chunks(SHA_EXTEND_ROUNDS * num_instance_per_batch))
            .zip(shard_ctx_vec)
            .flat_map(|((instances, steps), mut shard_ctx)| {
                let mut lk_multiplicity = lk_multiplicity.clone();

                instances
                    .chunks_mut(num_witin * SHA_EXTEND_ROTATION_ROUNDS)
                    .zip_eq(steps.chunks(SHA_EXTEND_ROUNDS))
                    .map(|(instance_with_rounds, steps)| {
                        let bh = BooleanHypercube::new(SHA_EXTEND_ROTATION_LOG2);
                        let cyclic_group = bh.into_iter().take(SHA_EXTEND_ROUNDS);

                        for (rotation_index, step) in izip!(cyclic_group, steps) {
                            let ops = step.syscall().expect("syscall step");

                            let rotation_index = rotation_index as usize;
                            let instance = &mut instance_with_rounds[rotation_index * num_witin..]
                                [..num_witin];
