...
impl<const M: usize, const C: usize, E: ExtensionField> UIntLimbs<M, C, E> {
    pub fn new<Name: Into<String>>(name: Name, circuit_builder: &mut CircuitBuilder<E>) -> Self {
    ...
    fn new_maybe_unchecked<Name: Into<String>>(
        ...
        circuit_builder.namespace(name, |cb| UIntLimbs {
    ...
    pub fn create_witin_from_exprs(
        ...
        let limbs = (0..Self::NUM_LIMBS)
            .map(|i| {
                ...
                circuit_builder.assert_ux::<_, C>("range check", w.expr());
                circuit_builder
    ...
    pub fn from_u8_limbs(x: &UIntLimbs<M, 8, E>) -> UIntLimbs<M, C, E> {
            ...
            .flat_map(|large_limb| {
                let limbs = (0..k)
                    .map(|_| {
                        ...
                        circuit_builder.assert_byte("", w.expr());
                        w.expr()
                    })
                    .collect_vec();
                ...
                circuit_builder.require_zero("zero check", large_limb.expr() - combined_limb);
                limbs
            })
            .collect_vec();
    ...
    pub fn as_lo_hi<const M2: usize>(&self) -> (UIntLimbs<M2, C, E>, UIntLimbs<M2, C, E>) {
        assert!(M == 2 * M2);
