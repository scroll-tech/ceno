...
/// Context construct from `E2EProgramCtx` and given inputs
pub struct E2ECtxWithInput<E: ExtensionField> {
    pub inner: Arc<E2EProgramCtx<E>>,
    pub init_full_mem: InitMemState,
}
...
impl<E: ExtensionField> E2EProgramCtx<E> {
    /// Setup input specified parameters
    pub fn setup_with_inputs(
        self: Arc<Self>,
        hints: &[u32],
        public_io: &[u32],
    ) -> Arc<E2ECtxWithInput<E>> {
        let io_init =
...
>(
    ...
    let start = std::time::Instant::now();
    let ctx = ctx.setup_with_inputs(hints, public_io);
    tracing::debug!("setup_with_inputs done in {:?}", start.elapsed());
    ...
    if let Checkpoint::PrepE2EProving = checkpoint {
        return E2ECheckpointResult {
            ...
            next_step: Some(Box::new(move || {
                _ = run_e2e_proof::<E, _>(ctx, pk, max_steps, is_mock_proving)
            })),
        };
    }
    ...
    if is_mock_proving {
        MockProver::assert_satisfied_full(
            &ctx.inner.system_config.zkvm_cs,
            ctx.inner.zkvm_fixed_traces.clone(),
            &zkvm_witness,
            &pi,
            &ctx.inner.program,
        );
