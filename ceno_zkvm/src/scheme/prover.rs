...
pub struct ZKVMProver<E: ExtensionField, PCS: PolynomialCommitmentScheme<E>, PB: ProverBackend, PD>
{
    ...
    device: PD,
    device_first_shard_pk: DeviceProvingKey<'static, PB>,
...
> ZKVMProver<E, PCS, PB, PD>
{
    pub fn new_with_single_shard(pk: ZKVMProvingKey<E, PCS>, device: PD) -> Self {
        ...
        let device_first_shard_pk = device.transport_proving_key(true, pk.clone());
    ...
    pub fn new(pk: ZKVMProvingKey<E, PCS>, device: PD) -> Self {
        ...
        // transport first shard proving key
        let device_first_shard_pk = device.transport_proving_key(true, pk.clone());
        // transport non first shard proving key
        let device_non_first_shard_pk = device.transport_proving_key(false, pk.clone());

        ZKVMProver {
            pk,
            device,
            device_first_shard_pk,
            device_non_first_shard_pk: Some(device_non_first_shard_pk),
            _marker: PhantomData,
        }
    }
    ...
    pub fn get_device_proving_key(
        ...
        if shard_ctx.is_first_shard() {
            &self.device_first_shard_pk
        } else {
            self.device_non_first_shard_pk
                .as_ref()
                .expect("please new prover with `new_with_multi_shards`")
        }
    }
}
...
> ZKVMProver<E, PCS, PB, PD>
{
    ...
    pub fn create_proof(
        ...
        let device_pk = self.get_device_proving_key(shard_ctx);
        let mut fixed_mles = device_pk.fixed_mles.clone();
