...
mod tests {
    ...
    fn test_infer_tower_witness() {
        ...
        let last_layer: Vec<ArcMultilinearExtension<E>> = vec![
            ...
            vec![E::from_canonical_u64(3u64), E::from_canonical_u64(4u64)].into_mle().into(),
        ];
    ...
    fn test_interleaving_mles_to_mles() {
        ...
        let input_mles: Vec<ArcMultilinearExtension<E>> = vec![
            vec![E::ONE, E::from_canonical_u64(2u64)].into_mle().into(),
            vec![E::from_canonical_u64(3u64), E::from_canonical_u64(4u64)].into_mle().into(),
            vec![E::from_canonical_u64(5u64), E::from_canonical_u64(6u64)].into_mle().into(),
            vec![E::from_canonical_u64(7u64), E::from_canonical_u64(8u64)].into_mle().into(),
        ];
    ...
    fn test_interleaving_mles_to_mles_padding() {
        ...
        let input_mles: Vec<ArcMultilinearExtension<E>> = vec![
            ...
            vec![E::from_canonical_u64(3u64), E::from_canonical_u64(0u64)].into_mle().into(),
            vec![E::from_canonical_u64(5u64), E::from_canonical_u64(0u64)].into_mle().into(),
        ];
    ...
    fn test_infer_tower_logup_witness() {
        ...
        assert_eq!(
            ...
            FieldType::Ext(vec![1.into_f(), 2.into_f(), 3.into_f(), 4.into_f()])
        );
        assert_eq!(
            layer[3].evaluations().clone(),
            FieldType::Ext(vec![5.into_f(), 6.into_f(), 7.into_f(), 8.into_f()])
        );
        ...
        assert_eq!(
            layer[0].evaluations().clone(),
            FieldType::<E>::Ext(vec![
                vec![1 + 5].into_iter().map(E::from_canonical_u64).sum::<E>(),
                vec![2 + 6].into_iter().map(E::from_canonical_u64).sum::<E>()
            ])
        );
        // next layer p2
        assert_eq!(
            layer[1].evaluations().clone(),
            FieldType::<E>::Ext(vec![
                vec![3 + 7].into_iter().map(E::from_canonical_u64).sum::<E>(),
                vec![4 + 8].into_iter().map(E::from_canonical_u64).sum::<E>()
            ])
        );
        // next layer q1
        assert_eq!(
        ...
        assert_eq!(
            ...
            // q12 * q11
            FieldType::<E>::Ext(vec![
                vec![(3 * 7) * 5].into_iter().map(E::from_canonical_u64).sum::<E>(),
            ])
        );
        // q2
        assert_eq!(
