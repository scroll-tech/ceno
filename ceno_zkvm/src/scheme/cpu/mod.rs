    ...
    for CpuProver<CpuBackend<E, PCS>>
{
    ...
    > {
        ...
        if let Some(gkr_circuit) = gkr_circuit {
            ...
            let (wits_in_evals, fixed_in_evals, main_sumcheck_proof, rt) = if num_instances
                .is_power_of_two()
            {
                ...
                let alpha_pows_expr = (2..)
                    .take(cs.w_table_expressions.len() + cs.r_table_expressions.len())
                ...
                let zero_check_expr: Expression<E> = cs
                    ...
                    .iter()
                    .take(1)
                    .chain(cs.r_table_expressions.iter().take(1))
                ...
                let fixed_in_evals = evals.split_off(cs.num_fixed);
                let _ = evals.split_off(cs.num_structural_witin as usize);
                let wits_in_evals = evals;
