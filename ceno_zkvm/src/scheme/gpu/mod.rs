...
impl GpuTowerProver {
    pub fn create_proof<'a, E: ExtensionField, PCS: PolynomialCommitmentScheme<E>>(
        ...
        fn merge_spec_witness<'b, E: ExtensionField, PCS: PolynomialCommitmentScheme<E>>(
            ...
            spec: TowerProverSpec<'b, GpuBackend<E, PCS>>,
            index: usize,
    ...
    for GpuProver<GpuBackend<E, PCS>>
{
    fn commit_traces<'a>(
        ...
        let largest_poly_size = traces
            ...
            .max()
            .unwrap();
        let _ = if let Some(s) = self.largest_poly_size
            && s >= largest_poly_size
        {
    ...
    for GpuProver<GpuBackend<E, PCS>>
{
    fn build_tower_witness<'a, 'b>(
        ...
        Vec<TowerProverSpec<'b, GpuBackend<E, PCS>>>,
        Vec<TowerProverSpec<'b, GpuBackend<E, PCS>>>,
    ) {
    ...
    fn prove_tower_relation<'a>(
        ...
        GpuTowerProver::create_proof(prod_specs, logup_specs, num_fanin, transcript)
    }
}
