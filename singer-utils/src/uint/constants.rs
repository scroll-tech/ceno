...
impl<const M: usize, const C: usize> UInt<M, C> {
    ...
    pub const N_OPERAND_CELLS: usize = (M + Self::MAX_CELL_BIT_WIDTH - 1) / Self::MAX_CELL_BIT_WIDTH;

    /// The number of `RANGE_CHIP_BIT_WIDTH` cells needed to represent one cell of size `C`
    pub const N_RANGE_CELLS_PER_CELL: usize = (Self::MAX_CELL_BIT_WIDTH + RANGE_CHIP_BIT_WIDTH - 1) / RANGE_CHIP_BIT_WIDTH;
