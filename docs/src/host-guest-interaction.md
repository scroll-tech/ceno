...
A critical aspect of developing ZK applications is understanding how the "host" (the machine running the prover) and
the "guest" (the ZK program running inside the vm) communicate with each other.

In Ceno, this communication happens in two main ways:

1. **Private Inputs (Hints)**: The host can pass private data to the guest.
2. **Public Inputs/Outputs (I/O)**: The guest can receive public data and commit to public outputs that the host can
   verify.
...
Private inputs, which Ceno refers to as "hints," are data known only to the host and the guest. They are not revealed
publicly and do not become part of the final proof. This is the primary way to provide secret inputs to your ZK program.
...
Public I/O is data that is known to both the host and the verifier. It is part of the public record and is used to
ensure the ZK program is performing the correct computation on the correct public data.
