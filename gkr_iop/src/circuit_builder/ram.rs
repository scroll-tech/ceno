use crate::{RAMType, error::CircuitBuilderError, selector::SelectorType};
...
impl<E: ExtensionField> CircuitBuilder<'_, E> {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn ram_type_write_with_rw_selectors<
        const LIMBS: usize,
        NR: Into<String>,
        N: FnOnce() -> NR,
    >(
        ...
        self.namespace(name_fn, |cb| {
            ...
            cb.with_selector(r_selector.clone(), |cb| {
                cb.read_record(|| "read_record", ram_type, read_record)
            })?;
            cb.with_selector(w_selector.clone(), |cb| {
                cb.write_record(|| "write_record", ram_type, write_record)
            })?;
