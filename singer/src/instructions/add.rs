...
impl<F: SmallField> Instruction<F> for AddInstruction {
    ...
    fn generate_wires_in(record: &Record) -> CircuitWiresIn<F> {
        assert_eq!(record.opcode, OpcodeType::ADD as u8);
        ...
        copy_operand_from_record!(wire_values, record, phase0_addend_0, 0);
        copy_operand_from_record!(wire_values, record, phase0_addend_1, 1);
        copy_range_values_from_u256!(
            wire_values,
            phase0_instruction_add,
...
mod test {
    ...
    /// for the add instruction circuit.
    #[test]
    fn test_interpreter_for_add_circuit() {
        let challenges = ChipChallenges::default();
        let bytecode = [
        ...
        let circuit_wires_in: Vec<Vec<Goldilocks>> = AddInstruction::generate_wires_in(&records[2])
            [0]
        .instances
        .clone();
        assert_eq!(circuit_wires_in.len(), 1);
        assert_eq!(circuit_wires_in[0].len(), AddInstruction::phase0_size());
