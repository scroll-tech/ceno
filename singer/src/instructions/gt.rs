...
impl<F: SmallField> Instruction<F> for GtInstruction {
    ...
    fn generate_wires_in(record: &Record) -> CircuitWiresIn<F> {
        assert_eq!(record.opcode, OpcodeType::GT as u8);
...
mod test {
    ...
    /// for the gt instruction circuit.
    #[test]
    fn test_interpreter_for_gt_circuit() {
        let challenges = ChipChallenges::default();
        let bytecode = [
        ...
        let circuit_wires_in: Vec<Vec<Goldilocks>> = GtInstruction::generate_wires_in(&records[2])
            [0]
        .instances
        .clone();
        assert_eq!(circuit_wires_in.len(), 1);
        assert_eq!(circuit_wires_in[0].len(), GtInstruction::phase0_size());
