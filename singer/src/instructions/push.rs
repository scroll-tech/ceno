...
mod test {
    ...
    fn bench_push1_instruction_helper<F: SmallField>(instance_num_vars: usize) {
        ...
        let size = PushInstruction::<1>::phase0_size();
        ...
        let _ = PushInstruction::<1>::construct_graph_and_witness(
            &mut singer_builder.graph_builder,
            &mut singer_builder.chip_builder,
            &circuit_builder.insts_circuits
                [<PushInstruction<1> as Instruction<F>>::OPCODE as usize],
            vec![phase0],
            &real_challenges,
            1 << instance_num_vars,
        ...
        println!(
            "Push1Instruction::construct_graph_and_witness, instance_num_vars = {}, time = {}",
            instance_num_vars,
        ...
        let _ = GKRGraphProverState::prove(&graph, &wit, &target_evals, &mut prover_transcript)
            .expect("prove failed");
        println!(
            "Push1Instruction::prove, instance_num_vars = {}, time = {}",
            instance_num_vars,
            timer.elapsed().as_secs_f64()
        );
    }
    ...
    fn bench_push1_instruction() {
        bench_push1_instruction_helper::<GoldilocksExt2>(10);
    }
}
