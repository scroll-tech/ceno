...
impl<F: SmallField, const N: usize> Instruction<F> for SwapInstruction<N> {
    ...
    fn generate_wires_in(record: &Record) -> CircuitWiresIn<F> {
        ...
        copy_stack_ts_lt_from_record!(
            wire_values,
            record,
            phase0_old_stack_ts_1,
            phase0_old_stack_ts_lt_1,
            0
        );
        copy_stack_ts_lt_from_record!(
            wire_values,
            record,
            phase0_old_stack_ts_n_plus_1,
...
mod test {
    ...
    /// for the swap instruction circuit.
    #[test]
    fn test_interpreter_for_swap_circuit() {
        let challenges = ChipChallenges::default();
        let bytecode = [
        ...
        let circuit_wires_in: Vec<Vec<Goldilocks>> =
            SwapInstruction::<1>::generate_wires_in(&records[2])[0]
                .instances
                .clone();
        assert_eq!(circuit_wires_in.len(), 1);
        assert_eq!(
            circuit_wires_in[0].len(),
            SwapInstruction::<1>::phase0_size()
        );

        #[cfg(feature = "witness-count")]
        {
            println!("SWAP: {:?}", &phase0_idx_map);
            println!("SWAP witness_size: {:?}", phase0_witness_size);
        }

        // initialize general test inputs associated with push1
        let inst_circuit = SwapInstruction::<1>::construct_circuit(challenges).unwrap();
