...
mod test {
    use crate::{
        instructions::InstructionGraph, scheme::GKRGraphProverState,
        test::test_opcode_circuit_with_witness_vector, SingerParams,
    };
    ...
    /// for the mstore instruction circuit.
    #[test]
    fn test_interpreter_for_mstore_circuit() {
        let challenges = ChipChallenges::default();
        let bytecode = [
        ...
        let mstore_circuit_wires_in: Vec<Vec<Goldilocks>> =
            MstoreInstruction::generate_wires_in(&records[2])[0]
                .instances
                .clone();
        assert_eq!(mstore_circuit_wires_in.len(), 1);
        assert_eq!(
            mstore_circuit_wires_in[0].len(),
            MstoreInstruction::phase0_size()
        );
