...
impl<F: SmallField> Instruction<F> for JumpInstruction {
    ...
    fn generate_wires_in(record: &Record) -> CircuitWiresIn<F> {
        assert_eq!(record.opcode, OpcodeType::JUMP as u8);
...
mod test {
    ...
    /// for the jump instruction circuit.
    #[test]
    fn test_interpreter_for_jump_circuit() {
        let challenges = ChipChallenges::default();
        let bytecode = [
        ...
        let circuit_wires_in: Vec<Vec<Goldilocks>> =
            JumpInstruction::generate_wires_in(&records[1])[0]
                .instances
                .clone();
        assert_eq!(circuit_wires_in.len(), 1);
        assert_eq!(circuit_wires_in[0].len(), JumpInstruction::phase0_size());
