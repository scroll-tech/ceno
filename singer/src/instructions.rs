...
use crate::{error::ZKVMError, CircuitWiresIn, InstWiresIn, SingerParams, SingerWiresIn};
...
fn circuit_wires_in_from_record<F: SmallField>(record: &Record) -> Vec<CircuitWiresIn<F>> {
    match OpcodeType::from_u8(record.opcode) {
        ...
        Some(OpcodeType::MSTORE) => vec![
            MstoreInstruction::generate_wires_in(record),
            MstoreAccessory::generate_wires_in(record),
        ],
...
pub(crate) fn construct_inst_graph_and_witness<F: SmallField>(
    ...
    let construct_circuit_graph = match OpcodeType::from_u8(opcode) {
        Some(OpcodeType::ADD) => AddInstruction::construct_graph_and_witness,
        Some(OpcodeType::GT) => GtInstruction::construct_graph_and_witness,
        Some(OpcodeType::CALLDATALOAD) => CalldataloadInstruction::construct_graph_and_witness,
        Some(OpcodeType::POP) => PopInstruction::construct_graph_and_witness,
        Some(OpcodeType::MSTORE) => MstoreInstruction::construct_graph_and_witness,
        Some(OpcodeType::JUMP) => JumpInstruction::construct_graph_and_witness,
        Some(OpcodeType::JUMPI) => JumpiInstruction::construct_graph_and_witness,
        Some(OpcodeType::JUMPDEST) => JumpdestInstruction::construct_graph_and_witness,
        Some(OpcodeType::PUSH1) => PushInstruction::<1>::construct_graph_and_witness,
        Some(OpcodeType::DUP1) => DupInstruction::<1>::construct_graph_and_witness,
        Some(OpcodeType::DUP2) => DupInstruction::<2>::construct_graph_and_witness,
