...
impl<F: SmallField> SingerCircuitBuilder<F> {
    pub(crate) fn new(challenges: ChipChallenges) -> Result<Self, ZKVMError> {
    ...
    pub(crate) fn construct_gkr_graph(
        ...
        let mut builder = SingerGraphBuilder::new();
        for (opcode, opcode_wires_in) in singer_wires_in.opcode_wires_in.iter_mut() {
            let inst_circuits = &self.insts_circuits[*opcode as usize];
        ...
        let (graph, graph_witness) = builder.graph_builder.finalize();
        Ok((
            SingerCircuit(graph),
...
#[derive(Clone, Debug)]
pub(crate) struct SingerWiresOutID {
...
/// output.
struct SingerGraphBuilder<F: SmallField> {
