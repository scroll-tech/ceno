...
pub fn i64_to_field<F: Field>(i: i64) -> F {
    if i < 0 {
        -F::from_u64(i.unsigned_abs())
    } else {
        F::from_u64(i as u64)
    }
}
...
/// coefficients.
pub(crate) fn interpolate_uni_poly<F: Field, E: Field + Mul<F, Output = E>>(
    ...
    for j in 1..len {
        let tmp = eval_at - E::from_u64(j as u64);
        evals.push(tmp);
        prod *= tmp;
    }
    let mut res = E::ZERO;
    ...
    for (j, p_i) in p_iter_rev.enumerate() {
        let i = len - j - 1;
        res += prod * p_i * denom_down * (evals[i] * denom_up).inverse();

        // compute denom for the next step is current_denom * (len-i)/i
        if i != 0 {
            denom_up *= -F::from_u64((j + 1) as u64);
...
mod test {
    ...
    fn test_eq_eval() {
        ...
        let expected = E::from_u64(119780);
    ...
    fn test_interpolate_uni_poly() {
        ...
        let eval_at = E::from_u64(11);
        let expected = E::from_u64(1610);
